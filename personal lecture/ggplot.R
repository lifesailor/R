...
# 1. One Numerical Variable - histogram + kernel density elstimator
...
# 2. One Categorical Variable - barplot
diamonds %>% 
  ggplot(aes(cut)) + 
  geom_bar()
...
round(prop.table(table(diamonds$cut))*100, 1) # percentage

diamonds %>%
  group_by(cut) %>%
  tally() %>% # tally works the same as summarise(n = n())
  mutate(pct = round(n/sum(n) *100, 1))



# 3. Two Continous Variable - scatter plot
diamonds %>%
...
# we pick sample for too many values
pairs(diamonds %>% sample_n(1000))



# 4. One Continous One Categorical - Boxplot
